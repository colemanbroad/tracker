...
float* imageFloat; // multipointer. holds pixel data.

// unsigned int* image; // multipointer. holds pixel data.

unsigned char* image;
unsigned int* height;
unsigned int* width;

void init(float* image0,
...
{
  unsigned int n = x0 + (*width)*y0;
  // image[n] = 1;
  if ((0<=x0) && (x0 < width[0]) && (0<=y0) && (y0 < height[0])) {imageFloat[n] = 1.0;}
  // if ((0<=n) && (n<(height[0]*width[0]))) 
}

void setPixelColor(int x0, int y0, unsigned int color)
{
  unsigned int n = x0 + (*width)*y0;
  image[n] = color;
}
...
void setPixelAA(int x0, int y0, float val)
{
  unsigned int n = x0 + (*width)*y0;
  imageFloat[n] = val;
}
...
void plotLineColor(int x0, int y0, int x1, int y1, unsigned int color)
{
    int dx =  abs(x1-x0), sx = x0<x1 ? 1 : -1;
    int dy = -abs(y1-y0), sy = y0<y1 ? 1 : -1;
    int err = dx+dy, e2;                                  /* error value e_xy */
...
{                                  /* plot a limited quadratic Bezier segment */
  ...
  // if (xx*sx <= 0 && yy*sy <= 0){
  //   printf("ERROR! LINE 215 ASSERT FAIL. \n");
  // }
...
//    float w = xd*(long)yd;
//    if (zd == 0){
//      plotEllipseRect(x0,y0, x1,y1);          /* looks nicer */
//      return;
//    }
